...
public class StayRegister {
	...
	public ModelAndView tolist(HttpServletRequest request, Integer currentPage, String txtname, Integer LvKeLeiXingId,
			Integer isBillID) {
	...
	public ModelAndView toteamlist(HttpServletRequest request, Integer currentPage, String txtname,
			Integer LvKeLeiXingId, Integer isBillID, Integer tuanDuiID, String teamNameId, String teamCodeId,
			String principalId, String contactPhoneNUmberId, String registerTimeId) {
		ModelAndView mv = null;
	...
	public ModelAndView confirmChangRoom(Integer id, Integer roomId, Double changRoomMoney, Timestamp changRoomTime,
			Integer LvKeLeiXingId) {
		ModelAndView mv = null;
		int chuZuFangShi = 0; // 出租方式
	...
	public ModelAndView toconsumption(Integer id, String lvKeName, Integer currentPage, Integer LvKeLeiXingId,
			Integer isBillID) {
		ModelAndView mv = null;
		fangjianId = id;
		lvkeName = lvKeName;
	...
	public ModelAndView consumption(String id, String Number, int consumptionStayRegisterID, Integer LvKeLeiXingId) {
		ModelAndView mv = null;
		Date date = new Date();
		DateFormat dformat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); // 设置日期格式
		Timestamp timestamp = Timestamp.valueOf(dformat.format(date)); // 将当前时间转为字符串
	...
	public ModelAndView toteampay(Integer tuanDuiID) {
		...
		Timestamp timestamp = Timestamp.valueOf(dformat.format(date)); // 将当前时间转为字符串
		List<AttributePo> listPayWay = attributeService.selectPayWay(); // 结账方式
		List<StayRegisterPo> listDeposit = stayRegisterService.selectTeamDeposit(tuanDuiID);// 根据id查询押金
		List<StayRegisterPo> listConsumption = stayRegisterService.selectTeamConsumption(tuanDuiID);// 根据id查询消费
		int receiveTargetID = stayRegisterService.selectReceiveTargetID(tuanDuiID);
		List<StayRegisterPo> list = stayRegisterService.selectFormTeamId(receiveTargetID, 68);
		for (int i = 0; i < listDeposit.size(); i++) {
			yaJin += listDeposit.get(i).getDeposit();
		}
		for (int i = 0; i < list.size(); i++) {
	...
	public Object timeAjaxSelectRoomTwo(Integer receiveTargetID, String datemin, String datemax) {
		 ...
		 * Map<String, Object> map=new HashMap<String, Object>(); map.put("canshu",
		 * "zhr"); map.put("teamPayVo", teamPayVo); list.add(map);
		 */
	...
	public Object timeAjaxSelectRoomThree(String id, Double zhuSuFei, Double huanFangFei, Double qiTaXiaoFei,
			Double jieZhangJinE, Double yaJin, Double yingBuJinE) {
		double ByaJin = 0;
		double BxiaoFei = 0;
		...
		for (int i = 0; i < FenGeId.length; i++) {
			...
			List<StayRegisterPo> xiaoFeiList = stayRegisterService
					.selectConsumptionJinJianBan(Integer.parseInt(FenGeId[i]));
