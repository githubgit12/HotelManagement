...
public class StayRegisterService implements com.gx.service.StayRegisterService {
	...
	@Override
	public Page<StayRegisterPo> pageFuzzyselectOne(int receiveTargeTypeID, int isBillID, String roomNumber,
			Page<StayRegisterPo> vo) {
		int start = 0;
		if (vo.getCurrentPage() > 1) {
			start = (vo.getCurrentPage() - 1) * vo.getPageSize();
		}
		List<StayRegisterPo> list = stayRegisterDao.pageFuzzyselectOne(receiveTargeTypeID, isBillID, roomNumber, start,
				vo.getPageSize());
		vo.setResult(list);
	...
	@Override
	public Page<StayRegisterPo> pageFuzzyselectTwo(int receiveTargetID, int isBillID, String roomNumber,
			Page<StayRegisterPo> vo) {
		int start = 0;
		if (vo.getCurrentPage() > 1) {
			start = (vo.getCurrentPage() - 1) * vo.getPageSize();
		}
		List<StayRegisterPo> list = stayRegisterDao.pageFuzzyselectTwo(receiveTargetID, isBillID, roomNumber, start,
				vo.getPageSize());
		vo.setResult(list);
	...
	public int selectReceiveTargetID(Integer tuanDuiID) {
		...
		return this.stayRegisterDao.selectReceiveTargetID(tuanDuiID);
	}
