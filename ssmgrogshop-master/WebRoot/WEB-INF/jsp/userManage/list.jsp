...
<!--   <link rel="stylesheet" href="${ctx}/css/roomset/roomset.css" type="text/css"></link> -->
  ...
  <div class="container" >
    <div class="span5">
	    <div class="row-fluid">
		    ...
		    <form action="${ctx}/userManage/tolist.do" method="post" style="float: left;">
	      ...
	      <tbody id="tbody">
	        <c:forEach items="${list.result}" var="item">
		        <tr>
		          ...
		          <td>${item.userName}</td>
		          <td>${item.password}</td>
		          <c:if test="${item.permissionId eq 1 }">
		          <td>管理员</td>
		          </c:if>
		          <c:if test="${item.permissionId eq 0 }">
		          <td>员工</td>
		          </c:if>
		        </tr>
	        </c:forEach>
 ...
 <script type="text/javascript">
   function addfunction(){
     parent.document.getElementById('Mainid').src='${ctx}/userManage/toadd.do';
   }
   ...
   var chk_value=[];
  	...
  	if(chk_value!=""){
		if(chk_value.toString().indexOf(",")>0){
		   ...
		   parent.document.getElementById("Mainid").src='${ctx}/userManage/toupdate.do?id='+chk_value;
		}
	}else{
	  alert("请选择一条数据进行修改");
	}
   ...
   var chk_value=[];
     ...
     if(flag){
  	  parent.document.getElementById("Mainid").src='${ctx}/userManage/delete.do?id='+chk_value;
  	}
  	}else{
  ...
  $(".tcdPageCode").createPage({
     ...
     location.href="${ctx}/userManage/tolist.do?currentPage="+p+"&txtname="+txtname;
     }
   });
